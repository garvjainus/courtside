...
# ---------------------
# Ball Tracker using a Kalman Filter
# ---------------------
class BallTracker:
    def __init__(self):
    ...
    def update(self, ball_center):
        if ball_center is None:
            # No new measurement available; just predict.
            return self.predict()
        measurement = np.array([[np.float32(ball_center[0])],
                                [np.float32(ball_center[1])]])
        if not self.initialized:
            # Initialize the state with the first measurement.
            self.kf.statePre = np.array([[ball_center[0]],
                                         [ball_center[1]],
                                         [0],
                                         [0]], np.float32)
...
# Instantiate the ball tracker globally.
ball_tracker = BallTracker()

def determine_possession(frame_detections):
    """
    Determines which person is likely in possession of the ball.
    If the ball is not detected due to occlusion, uses the ball tracker's prediction.
    Returns a tuple (user_id, center_position) or None if no suitable match.
    """
    # Use the detected ball if available; otherwise, rely on the tracker’s prediction.
    if frame_detections["ball"]:
        ...
        ball_center = ball_tracker.predict()

    # Update the tracker with the measurement (if available) to refine the prediction.
    ball_center = ball_tracker.update(ball_center if frame_detections["ball"] else None)

    min_dist = float('inf')
    possessor = None
    if not frame_detections["user_id"]:
        return None
    for detection in frame_detections["user_id"]:
        ...
        dist = ((ball_center[0] - user_center[0]) ** 2 + (ball_center[1] - user_center[1]) ** 2) ** 0.5
        if dist < min_dist:
            min_dist = dist
            possessor = (detection["user_id"], user_center)
    # Only return a possessor if the ball is close enough to a player's center.
    if min_dist < 100:
        return possessor
    return None
    ...
    while cap.isOpened():
        ...
        for box in results[0].boxes:
            ...
            if cls == 1:  # Person detected.
                user_detections = process_frame(frame)  # Process frame for person detection.
...
class PossessionTracker:
    ...
    PREV_POSSESSION_LIFETIME = 4       # Automatically null previous possession after 4 seconds.
    ...
    def process_frame(self, frame_detections, current_time):
        ...
        """
        # Determine possession based on ball location and nearby person detections.
        possessor = determine_possession(frame_detections)
        ...
        if possessor_id is not None:
            ...
            if self.current_possession is None:
                ...
                if possessor_id != self.current_possession:
                    # Check if the ball truly changed hands (to avoid triggering on minor dribbles).
                    if current_time - self.possession_timestamp > self.POSSESSION_CHANGE_THRESHOLD:
                        ...
                        # Too short a time difference – likely just a dribble.
                        self.possession_timestamp = current_time
                else:
        ...
        if frame_detections["rim"]:
            ...
            if shot_result is not None:
                if shot_result in ["2pt", "3pt"]:
                    ...
                    # A rebound is assumed if a person is detected shortly after a missed shot.
                    rebound = determine_possession(frame_detections)
    ...
    def process_shot(self, frame_detections):
        ...
        for rim in rim_detected:
            ...
            if (rim_box[0] <= ball_center[0] <= rim_box[2]) and (rim_box[1] <= ball_center[1] <= rim_box[3]):
                # Determine shot type based on distance from stored possession position to a fixed basket position.
                if self.stored_position is not None:
                    ...
                    # If the distance exceeds an arbitrary threshold, consider it a 3-pointer.
